        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-20">
            <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-md">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <input name="name" value={item.name} onChange={handleChange} placeholder="Name" className="w-full p-2 border border-slate-300 rounded focus:ring-blue-500 focus:border-blue-500" required />
                    <input name="price" type="number" value={item.price} onChange={handleChange} placeholder="Price" className="w-full p-2 border border-slate-300 rounded focus:ring-blue-500 focus:border-blue-500" required step="0.01" />
                    <input name="category" value={item.category} onChange={handleChange} placeholder="Category" className="w-full p-2 border border-slate-300 rounded focus:ring-blue-500 focus:border-blue-500" required />
                    <input name="imageUrl" value={item.imageUrl} onChange={handleChange} placeholder="Image URL" className="w-full p-2 border border-slate-300 rounded focus:ring-blue-500 focus:border-blue-500" required />
                    <div className="flex items-center">
                        <input id="available" name="available" type="checkbox" checked={item.available} onChange={handleChange} className="h-4 w-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500" />
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => { setEditingItem(null); setIsFormOpen(true); }} className="px-5 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium shadow-md hover:shadow-lg transition-shadow">Add New Item</button>
            </div>
                        ...
                        {isLoading ? (
                            ...
                            <tr key={item.id} className="hover:bg-slate-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-4">
                                    <button onClick={() => { setEditingItem(item); setIsFormOpen(true); }} className="text-blue-600 hover:text-blue-900">Edit</button>
